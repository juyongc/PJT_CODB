<template>
  <div class="col-6 col-md-3 col-lg-2 mx-3 my-5">
    <div class="card h-100" style="max-height: 25rem; background-color:rgb(35, 30, 30); border:1px solid rgb(35, 30, 30)">
      <img v-b-modal="movie.title" @click="showModal" class="movieImg"  :src="'https://www.themoviedb.org/t/p/original/' + movie.poster" alt="">
      
      <div class="card-footer border-0" style="padding:0%">

        <button class="cardBt btn btn-success d-inline" @click="goReviews" style="border-radius:0 0 0 0.5rem">
          <!-- <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-pencil-fill" viewBox="0 0 16 16">
          <path d="M12.854.146a.5.5 0 0 0-.707 0L10.5 1.793 14.207 5.5l1.647-1.646a.5.5 0 0 0 0-.708l-3-3zm.646 6.061L9.793 2.5 3.293 9H3.5a.5.5 0 0 1 .5.5v.5h.5a.5.5 0 0 1 .5.5v.5h.5a.5.5 0 0 1 .5.5v.5h.5a.5.5 0 0 1 .5.5v.207l6.5-6.5zm-7.468 7.468A.5.5 0 0 1 6 13.5V13h-.5a.5.5 0 0 1-.5-.5V12h-.5a.5.5 0 0 1-.5-.5V11h-.5a.5.5 0 0 1-.5-.5V10h-.5a.499.499 0 0 1-.175-.032l-.179.178a.5.5 0 0 0-.11.168l-2 5a.5.5 0 0 0 .65.65l5-2a.5.5 0 0 0 .168-.11l.178-.178z"/>
          </svg> 리뷰   -->
          Review
        </button>

        <button class="btn btn-danger cardBt" @click="byeWatchlist" style="border-radius:0 0 0.5rem 0">
          <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-trash-fill" viewBox="0 0 16 16">
          <path d="M2.5 1a1 1 0 0 0-1 1v1a1 1 0 0 0 1 1H3v9a2 2 0 0 0 2 2h6a2 2 0 0 0 2-2V4h.5a1 1 0 0 0 1-1V2a1 1 0 0 0-1-1H10a1 1 0 0 0-1-1H7a1 1 0 0 0-1 1H2.5zm3 4a.5.5 0 0 1 .5.5v7a.5.5 0 0 1-1 0v-7a.5.5 0 0 1 .5-.5zM8 5a.5.5 0 0 1 .5.5v7a.5.5 0 0 1-1 0v-7A.5.5 0 0 1 8 5zm3 .5v7a.5.5 0 0 1-1 0v-7a.5.5 0 0 1 1 0z"/>
          </svg>
        </button>   

      <Modal 
        v-if="isModal" 
        :movie="movieData"
        @close-modal="isModal = false"/>
      </div>
    </div>
  </div>
</template>

<script>
import Modal from '@/components/Modal.vue'
import axios from 'axios'
export default {
  name: 'WatchlistItem',
  components: {
    Modal,
  },
  props: {
    movie: {
      type: Object,
    }
  },
  data: function () {
    return {
      isModal: false,
      movieData: [],
    }
  },
  methods: {
    showModal: function () {
      this.isModal = true
      this.$store.dispatch('getImages', this.movie.movieid)
      this.$store.dispatch('getCredits', this.movie.movieid)
    },
    goReviews: function () {
      this.$router.push({ name: 'CreateReview' })
    },
    // delete 기능 추가
    setToken: function () {
      const token = localStorage.getItem('jwt')
      const config = {
        Authorization: `JWT ${token}`
      }
      return config
    },
    byeWatchlist: function () {
      axios({
        method: 'post',
        url: 'http://127.0.0.1:8000/accounts/',
        data: {
          title: this.movie.title,
          movieid: this.movie.movieid,
          poster: this.movie.poster
        },
        headers: this.setToken()
      })
      .then((res) => {
        this.like = res.data
        this.$router.go(this.$router.currentRoute)
      })
      .catch(err => {
        console.log(err)
      })
    },
  },
  created() {
    axios({
        method: 'get',
        url: `https://api.themoviedb.org/3/movie/${this.movie.movieid}?api_key=7ecf0fa910e1bacb146ddf503cf3ec72&language=ko-KR`
      })
      .then(res => {
        this.movieData = res.data
        return this.movieData
      })
      .catch(err => (
        console.log(err)
      ))
  }
}
</script>
<style>
.movieImg {
  /* width: 10%; */
  max-height: 100% !important;
  height: 100%;
  width: 100%;
}
.cardBt {
  width: 50%;
}
/* .mymovie-color {
  background-color: rgb(30, 30, 30) !important;
  border-color: rgb(30, 30, 30) !important;
} */
</style>